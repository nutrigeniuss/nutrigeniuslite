...
export function SummaryMetricCard({ icon, label, value, hint }: SummaryMetricCardProps) {
  return (
    <div
      title={hint}
      className="flex min-w-[140px] flex-1 items-center gap-3 rounded-2xl border border-slate-200/70 bg-white px-3.5 py-3 shadow-[0_8px_24px_rgba(15,23,42,0.04)]"
    >
...
export function MacroSection({ totalCal, weight, macros, setMacros, setLastInputMode, lastInputMode, compact = false }: MacroSectionProps) {
  ...
  return (
    <div className={`overflow-hidden border border-slate-200/70 bg-white shadow-[0_8px_24px_rgba(15,23,42,0.04)] ${compact ? 'rounded-[1.25rem]' : 'rounded-[1.5rem]'}`}>
      <div className={`flex flex-col gap-2 border-b border-slate-100 sm:flex-row sm:items-center sm:justify-between ${compact ? 'px-4 py-3' : 'px-5 py-3.5'}`}>
      ...
      <div className={`flex flex-col xl:flex-row ${compact ? 'gap-4 p-4' : 'gap-6 p-5'}`}>
        <div className={`flex-1 ${compact ? 'space-y-3' : 'space-y-3'}`}>
          {macroKeys.map((key) => {
            ...
            return (
              <div key={key} className={`border border-slate-100 bg-[#f7f8fc]/80 ${compact ? 'rounded-2xl p-2.5' : 'rounded-2xl p-3'}`}>
                <div className={`flex items-center gap-2 ${compact ? 'mb-1.5 flex-wrap' : 'mb-2'}`}>
                  ...
                  <input
                    ...
                    className="w-20 rounded-full border border-slate-200/90 bg-white px-2 py-1.5 text-center text-xs focus:border-brand-500 focus:outline-none focus:ring-2 focus:ring-brand-500/15"
                    placeholder="g"
                  />
